...
function bindPointerControls(canvas){
	// convert client coords -> canvas coords (handle CSS scaling)
	...
	canvas.addEventListener('touchstart', function(e){
		...
		e.preventDefault();
		var pos = clientToCanvas(canvas, e.touches[0].clientX, e.touches[0].clientY);
		TouchPointer.active = true;
		TouchPointer.x = pos.x;
		TouchPointer.y = pos.y;
	}, {passive:false});
	...
	canvas.addEventListener('mousedown', function(e){
		e.preventDefault();
		mouseDown = true;
		var pos = clientToCanvas(canvas, e.clientX, e.clientY);
		TouchPointer.active = true;
		TouchPointer.x = pos.x;
		TouchPointer.y = pos.y;
	});
...
function Peep(config,level){
	...
	self.update = function(){
		...
		// TOUCH / POINTER: if active, move toward pointer
		if(TouchPointer.active){
			var tdx = TouchPointer.x - self.x;
			var tdy = TouchPointer.y - self.y;
			var tdist = Math.sqrt(tdx*tdx + tdy*tdy);
			// if the pointer is sufficiently far, treat it like an input vector
			if(tdist > 4){
				dx = tdx / tdist;
				dy = tdy / tdist;
			} else {
		...
		// if touch controls push to left/right set direction accordingly
		if(TouchPointer.active){
			if(Math.abs(tdx) > 2){
				self.direction = (tdx < 0) ? -1 : 1;
			}
		}

		if(Key.left || Key.right || Key.up || Key.down || TouchPointer.active){
...
window.onload = function(){
	...
	onLoadAssets(function(){
		...
		window.level = new Level(window.INTRO_LEVEL);

		// bind touch/pointer controls for all canvases we use
		try {
			bindPointerControls(document.getElementById("canvas_intro"));
			bindPointerControls(document.getElementById("canvas_1"));
			bindPointerControls(document.getElementById("canvas_2"));
			bindPointerControls(document.getElementById("canvas_3"));
		} catch(e){
			// if canvases not found for any reason, skip binding (defensive)
			console.warn("pointer binding failed", e);
		}
